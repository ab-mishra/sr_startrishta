...
if( isset($_POST['action']) && $_POST['action']=='refreshOnlineUsers' )
{
	...
	{
	case 'inbox':
		//echo "SELECT added_uid as chat_uid FROMONUNION SELECT DISTINCT sent_by as chat_uid FROM `sr_user_message` m,`sr_users` u WHERE m.sent_to = '".$user_id."' AND m.sent_by = u.user_id AND u.user_name LIKE '%".$searchWord."%'";
	...
	while( $result = mysql_fetch_assoc($Query) )
	{
		...
		/* $html .= "<div class='list-group-item login-form left onlineUserDiv $onlineClass' id='onlineUserDiv-$onlineUserId'><div class='pic'>
		<a href='javascript:void(0);' id='chatOnlineUserList-$onlineUserId'><span class='list-img'><img src='$onlineUserImage' /></span></a>$matchIcon</div>
		<div class='name chatOnlineUserList'><a href='javascript:void(0);'>$onlineUser$onlineUserName</a>$unreadMsg</div>
		<div class='check_box msg_pro_chk'><div class='pull-left'><input id='checkbox$onlineUserId' type='checkbox' name='chatUserCheckbox' class='chatUserCheckbox' 
		value='$onlineUserId' $onlineCheckClass/><label for='checkbox$onlineUserId'><span class='pull-left'></span></label></div></div></div>"; 
		
		Updated by Kapil, 29/05/2017
		*/
		
		//$html .= "<li id='onlineUserDiv-$onlineUserId'><a href='javascript:void(0);' ><label id='chatOnlineUserList-$onlineUserId'><span class='list-img'><img src='$onlineUserImage'></span><i class='fa fa-circle text-success'></i><strong class='user-name'>$onlineUserName</strong></label><div class='pull-right check-box'>				 <span onclick='return delete_chat_func($onlineUserId);' class='delete_chat_head' > <i class='fa fa-trash text-success'</i></span></div></a></li>";
	}
...
############# REFRESH INLINE USER COUNT ###########
if(isset($_POST['action']) && $_POST['action']=='refreshOnlineUsersCount'){
	$totalUnreadMsgCount = 0;
	$msgType = $_POST['msgType'];
	$searchWord = $_POST['searchWord'];
	$chatUser_id = $_POST['chatUser_id'];
	...
	{
	case 'inbox':
		//echo "SELECT added_uid as chat_uid FROM sr_inbox_user WHERE user_id = '".$user_id."' UNION SELECT DISTINCT sent_by as chat_uid FROM `sr_user_message` m,`sr_users` u WHERE m.sent_to = '".$user_id."' AND m.sent_by = u.user_id AND u.user_name LIKE '%".$searchWord."%'";
		$Query = mysql_query("SELECT added_uid as chat_uid FROM sr_inbox_user WHERE user_id = '".$user_id."' 
		UNION SELECT DISTINCT sent_by as chat_uid FROM `sr_user_message` m,`sr_users` u WHERE m.sent_to = '".$user_id."' AND m.sent_by = u.user_id AND u.user_name LIKE '%".$searchWord."%' and m.is_deletebyto=0");
	break;
	case 'unread':
		//$Query=mysql_query("SELECT DISTINCT sent_by as chat_uid FROM `sr_user_message` WHERE sent_to='".$user_id."' AND is_read=0");
		$Query = mysql_query("SELECT count(DISTINCT sent_by) as chat_uid FROM `sr_user_message` m,`sr_users` u WHERE m.sent_to = '".$user_id."' AND m.sent_by = u.user_id AND is_read = 0 AND u.user_name LIKE '%".$searchWord."%'");
	break;
	case 'conversation':
		//$Query=mysql_query("SELECT DISTINCT sent_to as chat_uid FROM `sr_user_message` WHERE sent_by='".$user_id."'");
		$Query = mysql_query("SELECT count(DISTINCT sent_to) as chat_uid FROM `sr_user_message` m,`sr_users` u WHERE m.sent_by = '".$user_id."' AND m.sent_to = u.user_id AND u.user_name LIKE '%".$searchWord."%'");
	break;
	case 'online':
		//echo "SELECT count(DISTINCT user_id) as chat_uid FROM `sr_users` WHERE user_id != '".$user_id."' AND 	is_online = 1 and show_online = 1 AND user_name LIKE '%".$searchWord."%'";
		$userinfo=$userProfileObj->getUserInfo($user_id);
		$userLocation_lat=$userinfo['location_lat'];
		$userLocation_lon=$userinfo['location_lon'];
		$Query = mysql_query("SELECT DISTINCT user_id as chat_uid,(3959 * acos(cos(radians($userLocation_lat)) *
   cos(radians(location_lat)) *
   cos(radians(location_lon) -
   radians($userLocation_lon)) +
   sin(radians($userLocation_lat)) *
   sin(radians(location_lat )))
) AS distance FROM `sr_users` WHERE user_id != '".$user_id."' AND is_online=1 and show_online = 1 HAVING distance <= 62");
		/*$Query = mysql_query("SELECT count(DISTINCT user_id) as chat_uid FROM `sr_users` WHERE user_id != '".$user_id."' AND
		is_online = 1 and show_online = 1 AND user_name LIKE '%".$searchWord."%'");*/
	break;
	}
	if($msgType=='online') {
		echo $online_user_count = mysql_num_rows($Query);
	}
	else {
		while( $result = mysql_fetch_assoc($Query) )
        {
            $value = $result['chat_uid'];
            echo $value;
        }
	}
}
if(isset($_POST['action']) && $_POST['action']=='user_notInterested_action')
{
	$userid=$_POST['user_id'];
	$query=mysql_query("update sr_users set is_notInterestedChecked=1 where user_id=$user_id");
	echo 1;
}
############# Change Chat Listing ############
if( isset($_POST['action']) && $_POST['action']=='msgNavClicked' )
{
	...
	case 'inbox': ##Inbox
		...
		UNION
        SELECT  DISTINCT sent_to as inbox_uid_to
        FROM `sr_user_message`
        WHERE sent_by='".$user_id."' and is_deletebyfrom=0");
        break;
        case 'unread': ##Unread
   ...
   sin(radians(location_lat )))
    ...
    $html ='';
    $QueryInbox = mysql_query("SELECT added_uid as chat_uid_inbox
		FROM sr_inbox_user
		WHERE user_id='".$user_id."'
		UNION
		SELECT DISTINCT sent_by as chat_uid
        ...
        FROM `sr_user_message`
        WHERE sent_by='".$user_id."' and is_deletebyfrom=0");
//    $Array = mysql_fetch_array($QueryInbox);
   while($arr = mysql_fetch_array($QueryInbox))
   {
        $inboxdata[] = $arr['chat_uid_inbox'];
   }
    while( $result = mysql_fetch_assoc($Query) )
    {
		...
		<li id="onlineUserDiv-<?php echo $userInfoResult['user_id'];?>" >
			<a href="javascript:void(0);" >
				<label id="chatOnlineUserList-<?php echo $userInfoResult['user_id'];?>" >
			...
			<?php
				if( $userInfoResult['is_online'] != 1 || $userInfoResult['show_online'] != 1 || in_array($result['chat_uid'],$inboxdata) )
				{
					//echo '<i class="fa fa-circle text-success"></i>';
			?>
...
{	
	...
	while( $result = mysql_fetch_assoc($Query) )
	{
		...
		<li id="onlineUserDiv-<?php echo $result['user_id'];?>" >
			<a href="javascript:void(0);">
				<label id="chatOnlineUserList-<?php echo $result['user_id'];?>">
			...
			?>
				<div class="pull-right check-box delete_span">
				 <span  id="delete_chat_head-<?php echo $result['user_id']; ?>" onclick="return delete_chat_func(<?php echo $result['user_id']; ?>);" class="delete_chat_head" >
				  <i class="fa fa-trash text-success"></i></span>
					<!-- <input type="checkbox" id="checkbox<?php //echo $userInfoResult['user_id'];?>" name="chatUserCheckbox" value="<?php //echo $userInfoResult['user_id'];?>" />
					<label for="checkbox<?php //echo $userInfoResult['user_id'];?>"><span class="pull-left"></span></label> -->
				</div>
...
if(isset($_POST['action']) && $_POST['action']=='showUserChatProfile'){
	...
	$userMatchQuery=mysql_query("SELECT user_id,user_name,dob, gender, is_deleted, is_suspended, is_account_freeze, is_erase from sr_users WHERE 
		user_id='".$chatUserId."' ");
	$userMatchPhotoQuery=mysql_query("aSELECT * from sr_user_photo WHERE user_id='".$chatUserId."' LIMIT 0,3");
	...
	<div class="row">
		<div class="msg_cont_sec align-c" id="user_match">
			...
			<?php } ?>
				...
				<?php 
				if($userMatchResult['is_deleted']==0 || $userMatchResult['is_account_freeze']==0 || $userMatchResult['is_suspended']==0 || $userMatchResult['is_erase']==0 ){ ?> 
					<a href="profile.php?uid=<?php echo $chatUserId;?>" class="align-c green">View full profile here</a>
				<?php } else{ ?> 
					<a href="javascript:void(0)" class="align-c green">View full profile here</a>
				<?php } ?>
...
if( isset($_POST['action']) && $_POST['action']=='startConversation' )
{
	...
	<div class="chat-name-panel">
		...
		<div class="pull-left chat-head-content">
			<div class="pull-left chat-wap-title">
				<?php 
				//echo "SELECT * FROM sr_users WHERE user_id={$userMatchResult['user_id']}";
				$userInfoResult = mysql_fetch_assoc(mysql_query("SELECT * FROM sr_users WHERE user_id={$userMatchResult['user_id']}"));
				//echo $userInfoResult['is_account_freeze'].$userInfoResult['is_deleted'].$userInfoResult['is_suspended'].$userInfoResult['is_erase'];
					//print_r($userInfoResult);
				if($userInfoResult['is_deleted']==0 && $userInfoResult['is_account_freeze']==0 && $userInfoResult['is_suspended']==0 &&	 $userInfoResult['is_erase']==0 )
				{ ?> 
					<a href="profile.php?uid=<?php echo $userMatchResult['user_id'];?>" ><?php echo $chatUserName; ?>, <?php echo $chatUserAge; ?></a>
				<?php 
				} 
				else 
				{ ?>
					<a href="javascript:void(0)" ><?php echo $chatUserName; ?>, <?php echo $chatUserAge; ?></a>
				<?php }
				?>
			</div>
			<div class="pull-left chat-wap-title">
	...
	//echo '<script>alert("'.$chatLimitCount.'")</script>';
	...
	if( $isProfileImage )
	{
		...
		if( $isChatUserRandom && $rec_count == 0 && $send_count == 0 && !isset($_POST['clicked']) )
		{
			...
			<div class="chat-sec-panel-3" id="scroll-bot" >
				<div class="msg_cont_sec2 align-c">
					...
					<p class="pull-left full margin-t5"><a href="javascript:void(0)" id="not_interested" style="border-bottom:1px dotted #000;"><input type="hidden" name="sentby" class="sentby" value="<?php echo $chatUserId ?>">Not Interested</a></p>
				</div>
			</div>
			<?php 
		}
		...
		{
			//echo "<script>alert('No Vip');</script>";
				...
				<div class="chat-sec-panel-3" id="scroll-bot" >
					<div class="msg_cont_sec2 align-c" style="height:440px; margin: 0 6px;">
						<div class="msg_cont_sec2 align-c" style="height:434px;margin: 0 6px;">
							...
							<div class="margin-b10"><p>Upgrade to a VIP membership and get exclusive access <br />to every new user the moment they join!</p></div>
			...
			else if( mysql_num_rows($chatUserQuery) == 0 && $rec_count == 0 && $send_count == 0 && $userDiffJoinDay != 0 )
			{
				...
				<div class="chat-sec-panel-3" id="scroll-bot" style="height:calc(100vh - 216px);">
					<div class="msg_cont_sec2 align-c" style="min-height: 430px;">
						...
						<h3 class="margin-t20 margin-b20 lh-40">You don&#39;t match <?php echo $userPerNown; ?> search criteria</h3>
						<div class="margin-b30"><p>
                                Still want to chat to <?php echo $userPer;?> ? Become a VIP member and get unlimited access to every profile!</p></div>
                        <br /><br />
						<div class="align-c margin-t5 margin-b20 full pull-left">
			...
			{
				//echo "<script>alert('".$newMsgCountQuery."s');</script>";
				...
				<div class="chat-sec-panel-3" id="scroll-bot" style="height:calc(100vh - 130px);">
					<div class="msg_cont_sec2 align-c" id="msg-reply-div">
						...
						<div class="full clearfix">
							...
							<?php
							$deleted_msg_query=mysql_query("SELECT * FROM `sr_user_message` WHERE `sent_to` =$user_id   AND `sent_by` =$chatUserId and is_deletebyto=1") or die(mysql_error());
							 $deleted_msg_count=mysql_num_rows($deleted_msg_query);
							?>
							<div class="full margin-t20 replay_wap">
								...
								<!-- Check 4-->
								<div class="align-c margin-t5 margin-b20 replay_wap_btn">
								<a href="javascript:void(0);" id="msg_nointerest" class="btn btn-default custom bg_gray wd_200">I'm not interested
									<input type="hidden" id="user_notInterested" value="<?php echo $userProfileObj->getUserNotInterested($user_id) ?>" />
									<input type="hidden" name="sentbyuser" id="chatuserid" class="sentby" value="<?php echo $chatUserId ?>" />
									<input type="hidden" id="del_count" value="<?php echo $deleted_msg_count ?>" />	
						...
						<div class="full clearfix">
							...
							<div class="user_pics pull-left">
								<ul class="clearfix mess-wap-cen">
									...
									while( $userPhotoResult = mysql_fetch_assoc($userPhotoQuery) )
									{
										...
										{
											?>
											<li><a><img src="upload/photo/<?php echo $userPhotoResult['photo'];?>"></a></li>
											<?php
											$pi++;
										}
									}
							...
							<p>
							<!--<a href="javascript:void(0);" id="not_interested"><input type="hidden" name="sentby" class="sentby" value="<?php //echo $chatUserId ?>">Not Interested</a> -->
							</p>
						</div>
					</div>
				</div>
				...
				<div class="msg_cont_sec2 align-c" id="msg-reply-div">
					...
					<div class="full clearfix">
						...
						<div class="full margin-t20 replay_wap">
							...
							<!-- Check 3-->
							<div class="align-c margin-t5 margin-b20 replay_wap_btn">
								<a href="javascript:void(0);" id="msg_nointerest" class="btn btn-default custom bg_gray wd_200">
								<input type="hidden" id="user_notInterested" value="<?php echo $userProfileObj->getUserNotInterested($user_id); ?>" />
									<input type="hidden" name="sentbyuser" id="chatuserid" class="sentby" value="<?php echo $chatUserId; ?>" />
									<input type="hidden" id="del_count" value="<?php echo $deleted_msg_count; ?>" />
									I'm not interested </a>
		...
		elseif( $chatLimitCount == 5 && (!in_array($chatUserId ,$startChatUserArray)) )
		{
			...
			<div class="chat-sec-panel" id="scroll-bot" style="height: calc(100vh - 274px);">
				<div class="msg_cont_sec align-c">
					<div class="msg_cont_sec align-c">
						<div class="user_pics pull-left">
			...
			<div class="chat-wap-c">
				<div class="send-chat">
					<div class="user-name-c">
						<a href="#"><span><?php echo  $showChatResult['user_name']; ?></span>
							<i>
							<?php
...
if(isset($_POST['action']) && $_POST['action']=='deleteConversation'){
	...
	$Query1=mysql_query("UPDATE sr_user_message SET is_deletebyfrom=1 WHERE sent_by='".$user_id."' AND sent_to='".$deleteUserId."'") or die(mysql_error());
	$Query2=mysql_query("UPDATE sr_user_message SET is_deletebyto=1 WHERE sent_to='".$user_id."' AND sent_by='".$deleteUserId."'") or die(mysql_error());
		
	if($Query1 && $Query2){
		// /echo "UPDATE sr_user_message set is_read=1 WHERE sent_to = $user_id AND sent_by = $deleteUserId";
		$reset_unread_message_query=mysql_query("UPDATE sr_user_message set is_read=1 WHERE sent_to = $user_id AND sent_by = $deleteUserId");
		echo 1;
	}else {
		echo 0;
	}
...
{
	echo "UPDATE sr_user_message SET is_deletebyfrom = 1 WHERE sent_by = '".$user_id."'";
	$Query1 = mysql_query("UPDATE sr_user_message SET is_deletebyfrom = 1,is_read=1 WHERE sent_by = '".$user_id."'") or die(mysql_error());
	$Query2 = mysql_query("UPDATE sr_user_message SET is_deletebyto = 1,is_read=1 WHERE sent_to = '".$user_id."'") or die(mysql_error());
