...
class Admin extends Connection{
	...
	/*function getProfileImage($profile_image)
	{
		...
		else if( ! file_exists(SITEURL."upload/photo/thumb/$profile_image"))
		{
			return SITEURL."upload/profile_images/dummy-profile.png";
		}else
		{
	...
	function getProfileImage($profile_image){
		...
		//echo SITEURL."upload/photo/$profile_image"."<br/>";
	...
	function getThumbProfileImage($profile_image){
		//echo $profile_image."<br/>";
		//echo SITEURL."upload/photo/$profile_image"."<br/>";
			if (file_exists(SITEURL."upload/photo/$profile_image")) {
	...
	function approveInterest($interest_id){
		...
		$res=0;
		/*echo "herer";die;*/
		$query= mysql_query("UPDATE sr_interest SET status=1, approved_on = '".DATE_TIME."' WHERE interest_id='".$interest_id."'");
		/*Updated By Chitra, 05/06/2017*/
		$query1 = mysql_query("UPDATE sr_user_interest SET status='1', `updated_on` = '".DATE_TIME."' WHERE interest_id='".$interest_id."'");
		if($query){
			$res=1;	
	...
	function updateUserInterest($interest_id, $cat_id, $interest, $user_id){
		...
		$res=0;
		if($interest_id != 0 && $cat_id !=0 && $interest != ''){
			$query=("UPDATE sr_interest SET interest='".$interest."', cat_id='".$cat_id."' WHERE interest_id='".$interest_id."'");
	...
	function getUnreadPhotoReported($searchKey =''){
		...
		$query=mysql_query("SELECT r.* , p.user_id , p.photo FROM  
			`sr_photo_report` r, sr_user_photo p , sr_user_login u 
			WHERE p.`photo_id` = r.`photo_id` AND r.status=1 
			AND r.is_read='0' AND r.disapprove_by_admin=0 
			AND p.user_id = u.user_id $addQuery ");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
	...
	function getReadPhotoReported($searchKey=''){
		...
		$query=mysql_query("SELECT r.*, p.user_id, p.photo FROM 
			`sr_photo_report` r, sr_user_photo p, sr_user_login u 
			WHERE p.`photo_id` = r.`photo_id` AND r.is_read = '1' 
			AND r.disapprove_by_admin = 0  AND p.user_id = u.user_id 
			$addQuery order by photo_report_id desc ");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
    ...
    function getPhotoReported(){
		$array = array();
		$query=mysql_query("SELECT r.* FROM  `sr_photo_report` r, `sr_user_photo` p, sr_user_login u  
			WHERE p.`photo_id` = r.`photo_id` 
			AND r.`is_read` = '0' 
			AND r.disapprove_by_admin = '0'  
	...
	function getUnreadUserReported(){
		$array = array();
		$query = mysql_query("SELECT sr_user_report.user_id, sr_users.user_name,
			sr_user_report.user_report_id,
			sr_user_report.reported_by,
			sr_user_report.reason,
			sr_user_report.description,
			sr_user_report.reported_on
			FROM sr_user_report LEFT JOIN sr_users on sr_users.user_id = sr_user_report.user_id 
			WHERE sr_user_report.status=1 AND sr_user_report.is_read=0 
			order by sr_user_report.user_report_id desc ");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
	...
	{
		$array = array();
		$query = mysql_query("SELECT * FROM sr_user_report WHERE 
			status=1 AND is_read=1 order by user_report_id desc ");
		if( $query )
		{
			while( $result = mysql_fetch_assoc($query) )
			{
    ...
    function getAbuseReport(){
		$array = array();
		$query=mysql_query("SELECT * FROM sr_user_report WHERE status=0");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
	...
	function getSearchUsers($searchType, $searchValue, $position, $item_per_page){
		...
		//echo "Where ----- ".$whereclause;die;
		$query=mysql_query("SELECT * FROM sr_users u , sr_user_login ul where ".$whereclause." ORDER BY u.created_on DESC LIMIT $position, $item_per_page");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
	...
	function getOnlineUsers(){
		$array = array();
		$query=mysql_query("SELECT * FROM sr_users u , sr_user_login ul where u.user_id=ul.user_id AND u.is_online=1 AND u.is_deleted=0 AND u.is_suspended=0 and ul.email_id!=''");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
    ...
    function getNewUsers(){
		$array = array();
		$query=mysql_query("SELECT * FROM sr_users u , sr_user_login ul where u.user_id=ul.user_id AND u.`created_on` >= '".DATE_TIME."' - INTERVAL 1 DAY and ul.email_id!=''");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
    ...
    function getDeletedUsers(){
		$array = array();
		$query=mysql_query("SELECT * FROM sr_users u , sr_user_login ul where u.user_id=ul.user_id 
			AND u.is_deleted=1 and ul.email_id!=''");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
    ...
    function getNotVerifiedUsers(){
		$array = array();
		$query=mysql_query("SELECT * FROM sr_users u , sr_user_login ul where u.user_id=ul.user_id AND u.is_verified=0 and ul.email_id!=''");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
    ...
    function getNoProfilePhotoUsers(){
		$array = array();
		$query=mysql_query("SELECT * FROM sr_users u , sr_user_login ul where u.user_id=ul.user_id AND u.profile_image='' and ul.email_id!=''");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
    ...
    function getTotalUsers(){
		$array = array();
		$query=mysql_query("SELECT * FROM sr_users WHERE gender=1 OR gender = 2");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
	...
	function getUserdeleteReason($user_id){
		$res = array();
		//echo $user_id;
		$query = mysql_query("SELECT reason,description, deleted_on FROM `sr_delete_profile` 
			WHERE user_id = '".$user_id."' AND status=1");
		if($query){
			while($result = mysql_fetch_assoc($query)){
				$res['reason'] = $result['reason'];
				$res['time'] = $result['deleted_on'];
			}
		}
		return $res;
	}
	...
	function getUserPrivileges($user_id){
		$query=mysql_query("SELECT * FROM sr_admin_privilege WHERE admin_id='".$user_id."' AND status=1");
	...
	function suspendUser($userId, $user_name, $reason, $description, $report_id=''){
		...
		if($query){
			$sql = mysql_query("INSERT INTO sr_user_suspend SET user_id='".$userId."', reason='".$reason."', description='".$description."', 
			suspended_by='".$admin_id."', suspended_on=now() , status=1");
			
			$timestampQuery = mysql_query("INSERT INTO sr_admin_user_timestamp SET user_id='".$userId."', 
			activity='Suspanded User', admin_id='".$admin_id."', timestamp=now() , status=1, report_id='".$report_id."'");
			
			//EMAIL
			$to = $this->getUserEmailId($userId);
			...
			$sendMail = $this->sendTemplateMail($to, $from, $from_name, $subject, $header, $body, $footer);
			$response='1';
