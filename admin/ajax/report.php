...
if( isset($_POST['action']) && $_POST['action']=='refreshApprovedPhotosDiv' )
{
	...
	if( $getLastTwoApprovedPhotos!=0 )
	{
		while( $getLastTwoApprovedPhotosFetch = mysql_fetch_array($getLastTwoApprovedPhotos) )
		{
			...
			$imgPath = SITEURL."upload/profile_images/dummy-profile.png"; /*Updates by Chitra 8/5/2017*/
			if($getLastTwoApprovedPhotosFetch['photo'] != '')
			{
				if( file_exists(SITEURL."upload/photo/".$getLastTwoApprovedPhotosFetch['photo']) )
				{
					$imgPath = SITEURL."upload/photo/".$getLastTwoApprovedPhotosFetch['photo'];
				}
				else{
					$imgPath = SITEURL."upload/photo/".$getLastTwoApprovedPhotosFetch['photo']; /*Updates by Chitra 8/5/2017*/
				}
			}
			?>
	...
	<div class="clearfix"></div>
		<ul class="clearfix gallery new-interest">
			...
			foreach( $newPhotosArray as $newPhoto )
			{
				...
				?>
				<li id="photoDiv-<?php echo $newPhoto['photo_id'];?>" data="<?php echo $newPhoto['photo'];?>">
					<ul class="clearfix hide-show">
						<li>
							...
							<a href="javascript:void(0);" rel="prettyPhoto[gallery1]" >
								...
								if($newPhoto['photo'] != '')
								{
									if( file_exists(SITEURL."upload/photo/".$newPhoto['photo']) )
									//if( file_exists("../../upload/photo/".$newPhoto['photo']) )
									{
										//echo '1';
										$imgPath = SITEURL."upload/photo/".$newPhoto['photo'];	
									}
									else{
										//echo '2';
										//echo SITEURL."upload/photo/".$newPhoto['photo'];	
										$imgPath = SITEURL."upload/photo/".$newPhoto['photo'];	
									}
								}
								?>
