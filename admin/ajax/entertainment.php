...
$allusers=$epObj->getAllUsers($time_limit);
$maleUsers=$epObj->getMaleUsers($time_limit);
$femaleUsers=$epObj->getFemaleUsers($time_limit);
$insertCount=0;
if( isset($_POST['action']) && trim($_POST['action'])=="createEP" )
{
	...
	$lan1=$_POST['locationlat'];
	$lon1=$_POST['locationlon'];
	$lan2=$_POST['locationlat_1'];
	$lon2=$_POST['locationlon_1'];
	$lan3=$_POST['locationlat_2'];
	$lon3=$_POST['locationlon_2'];
	$lan4=$_POST['locationlat_3'];
	$lon4=$_POST['locationlon_3'];
	$lan5=$_POST['locationlat_4'];
	$lon5=$_POST['locationlon_4'];
	$lan6=$_POST['locationlat_5'];
	$lon6=$_POST['locationlon_5'];

	$dob_date=date('Y-m-d',strtotime($dob_date));
	$insertID = $epObj->createProfile($usr_name, $dob_date, $gender, $location, $aboutMe, $lookingFor, $relation, $starsign, $sexuality, $bodytype, $complexion, $height, $eyecolor, $haircolor, $language, $smoking, $drinking, $kids, $education, $usr_work, $usr_iamhere,$lan1,$lon1,$lan2,$lon2,$lan3,$lon3,$lan4,$lon4,$lan5,$lon5,$lan6,$lon6);
...
if( isset($_POST['action']) && trim($_POST['action'])=="deleteProfile" )
{
	...
	$query=('delete from sr_ep_profile where user_id='.$userid);
	//echo "Affected -----".mysql_affected_rows($query);//die;
...
if(isset($_POST['action']) && trim($_POST['action'])=='stock')
{
	...
	if($_POST['message-select']=='male')
	{
		foreach ($maleUsers as $maleUser) {
			...
			//echo "SELECT * FROM `sr_user_message` WHERE is_deletebyto>0 AND `sent_to` =$muid   AND `sent_by` =$userId ";
			$sql3=mysql_query("SELECT * FROM `sr_user_message` WHERE is_deletebyto>0 AND `sent_to` =$muid   AND `sent_by` =$userId") or die(mysql_error());
			//echo mysql_num_rows($sql);
			//echo mysql_num_rows($sql2);
			//echo mysql_num_rows($sql3)."   ".$maleUser['user_id']."</br>";
            $stockMessage = $_REQUEST['stockMessage'];
			if((mysql_num_rows($sql)==0 && mysql_num_rows($sql2)==0) || mysql_num_rows($sql3)>0)
			{
			//	echo "INSERT INTO `sr_user_message` (`msg_id`, `msg`, `msg_upload`, `sent_to`, `sent_by`, `sent_date`, `receive_date`, `status`, `is_read`, `is_deletebyfrom`, `is_deletebyto`, `notify_status`) VALUES (NULL, 'Hey nice profile..!', '', $muid, $userId, now(), '', '1', '0', '0', '0', '0'); ";

                if($stockMessage=='')
                {
                    $stockMessage='Hey nice profile..!';
                }

				$insertMessageQuery=mysql_query("INSERT INTO `sr_user_message` (`msg_id`, `msg`, `msg_upload`, `sent_to`, `sent_by`, `sent_date`, `receive_date`, `status`, `is_read`, `is_deletebyfrom`, `is_deletebyto`, `notify_status`) VALUES (NULL, ".$stockMessage.", '', $muid, $userId, now(), '', '1', '0', '0', '0', '0')") or die(mysql_error());
