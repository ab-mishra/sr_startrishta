			...
			<div class="modal-dialog pop-up-width" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="text-center clearfix">
							<button type="button" class="btn btn-popup-red" id="upgrade-vip">Upgrate TO VIP</button>
						</div>
					</div>
				</div>
			</div>
			...
			$.ajax({
				...
				success:function(result){
					...
					if(result == 1){
						...
						$('#error_message').html('User unsuspanded sucessfully');
		...
		$('#upgrade-vip').on('click' , function(){
			...
			$.ajax({
				...
				success:function(result){
					...
					if(result == 1){
						$('#error_message_header').html('Alert');
						$('#error_message').html('upgrade to VIP sucessfully');
						$('#alert_modal').modal('show');
					}else if(result == 0){
						$('#error_message_header').html('Alert');
						$('#error_message').html('Problem in network.please try again.');
