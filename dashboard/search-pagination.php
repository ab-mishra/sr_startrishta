...
function getProfileImage($profile_image){
		if($profile_image == ''){
			return "upload/photo/dummy-profile.png";
...
foreach($userResult as $user){
	...
	?>
		<tr>
			...
			<td>
				<img src="<?php echo $adminObj->getProfileImage($user['profile_image']);?>" class="img-responsive powersearch-img"/>
			</td>
			...
			<td> 
				<div class="btn-group button-edit">
					...
					<ul class="dropdown-menu info right" role="menu">
						...
						<?php if(!$isUserVip){?>
						<li><a href="javascript:void(0);" class="vip-modal" id="upgrade_vip-<?php echo $userId.'-'.$userName;?>"> Upgrate  to VIP</a></li>
						<?php } ?>
