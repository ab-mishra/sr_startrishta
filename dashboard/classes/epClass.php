...
class Entertainment extends Connection
{
	...
	{
	//	print_r($_POST);
		//echo "Insert into sr_user_login(email_id, password, role_id, status, created_on) values('', '', '2', '1', '".date('Y-m-d')."') ";
		...
		//	echo "insert into sr_users(user_id, user_name, gender, dob, location, relationship_status, sexuality, star_sign, body_type, complexion, height, height_in, eye_color, hair_color, language, smoking, drinking, kids, education, work, about_me, looking_for, here_to, created_on) values('".$lastUserID."', '".$usr_name."', '".$gender."', '".$dob_date."', '".$location."', '".$relation."', '".$sexuality."', '".$starsign."', '".$bodytype."', '".$complexion."', '".$height."', '1', '".$eyecolor."', '".$haircolor."', '".$language."', '".$smoking."', '".$drinking."', '".$kids."', '".$education."', '".$usr_work."', '".$aboutMe."', '".$lookingFor."', '".$usr_iamhere."', '".date('Y-m-d')."') ";
		...
		if(isset($_FILES["imagefile"]["type"][0]))
		{
			 ...
			 $sourcePath = $_FILES['imagefile']['tmp_name'][0]; // Storing source path of the file in a variable
						 $targetPath = "../../upload/photo/".$_FILES['imagefile']['name'][0]; // Target path where file is to be stored
