...
class Admin extends Connection{
    ...
    function getPhotoReported(){
		$array = array();
		$query=mysql_query("SELECT r.* FROM  `sr_photo_report` r, `sr_user_photo` p WHERE p.`photo_id` = r.`photo_id` AND r.`status`=0");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
	...
	function getUnreadUserReported(){
		...
		$query = mysql_query("SELECT sr_user_report.user_id, sr_users.user_name,
			...
			WHERE sr_user_report.status=1 AND sr_user_report.is_read=0 order by sr_user_report.user_report_id desc ");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
	...
	function getSearchUsers($searchType, $searchValue, $position, $item_per_page){
		$array = array();
		if($searchType == ''){
			$query=mysql_query("SELECT * FROM sr_users u , sr_user_login ul where u.user_id=ul.user_id ORDER BY u.uid DESC LIMIT $position, $item_per_page");
		}else{
			if($searchType == 'online'){
				$query=mysql_query("SELECT * FROM sr_users u , sr_user_login ul where u.user_id=ul.user_id AND u.is_online=1  ORDER BY dob DESC LIMIT $position, $item_per_pa	ge");
			}
    ...
    function getNewUsers(){
		$array = array();
		$query=mysql_query("SELECT * FROM sr_users u , sr_user_login ul where u.user_id=ul.user_id and ul.email_id!='' AND u.`created_on` >= now() - INTERVAL 1 DAY");
		if($query){
			while($result=mysql_fetch_assoc($query)){
				$array[] = $result;
			}
