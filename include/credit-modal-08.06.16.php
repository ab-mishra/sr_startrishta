...
$credit_cancel_url=HTTP_SERVER.'paypal-credit-cancel.php'; // Test Paypal API URL
...
$responseHtml = '<span class="input_box fontawesome-select select-payment">&nbsp;&nbsp;'.$defaultCurrencyIcon.' '.$defaultCreditAmount.' - '.$defaultCredit.' credits</span>
					<span class="down_arrow_btn fontawesome-select"> <b>&#xf0d7; </b></span>
					<div class="clearfix"></div>
					<ul class="p_locations clearfix fontawesome-select credit-payment-options">';
...
$responseHtml .= '<li class="p_location payment_total" id="'.$result['id'].'-'.$defaultCurrencyIcon.'">
				<a href="javascript:void(0)">&nbsp;&nbsp;'.$defaultCurrencyIcon.' '.$result['price'].' - '.$result['credit'].' credits</a>
				</li>';		
	}
$responseHtml .= '</ul>';
		...
		<div class="relative">
			<div class="filter1 margin-l90 margin-b15">
				<div class="form-group">
					...
					<span class="input_box fontawesome-select">&nbsp; $ - US Dollars, USD</span>
					<span class="down_arrow_btn fontawesome-select"> <b>&#xf0d7; </b></span>
					<div class="clearfix"></div>
...
$paypalFooter = '<div class="pay-footer">
		<hr>
		<p class="footer"><span class="paypal-foot-msg">You will be charged '.$defaultCurrencyIcon.' '.$defaultCreditAmount.' by PayPal. </span>   <a href="javascript:void(0)" data-toggle="modal" data-target="#credit_service_condition" class="color-green"> Service conditions</a></p>
	</div>';
		...
		$('.payment_total').live('click' , function(){
			...
			$.ajax({
				...
				success:function(result){
					...
					$(".credit_return").val(http_path+'paypal-credit-success.php?ID='+result[2]+'&credit='+credit_for);
					$(".currency_code").val(result[1]);
					$(".paypal-foot-msg").html('You will be charged '+icon+' '+result[0]+' by PayPal.');
				}
			});
			$('#preloader').hide();
			return false;
