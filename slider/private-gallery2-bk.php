		...
		<div id="my-gallery-modal-<?php echo $privatePhotoId2.'-'.$iCount;?>" class="modal fade login_pop" role="dialog" >
			<div class="modal-dialog myphotos">
				<div class="modal-content">
					<div >
					<?php 
					$ph_count=1;
					foreach($privateGalleryArray2 as $gallery1){
						$ph_id1=$gallery1['photo_id'];
						if($privatePhotoId2 == $ph_id1) $head_class='block'; else $head_class='none';
					?>
						<div class="modal-header modal-header-<?php echo $privatePhotoId2.'-'.$iCount;?>" id="header-<?php echo $ph_id1.'-'.$iCount;?>" style="display:<?php echo $head_class;?>">
							<button type="button" class="close" data-dismiss="modal">&times;</button>
							<h4><b>Private Photos </b><span class="brown-bg"> <?php echo $ph_count;?>/<?php echo $privatePhotoCount2;?> </span></h4>
						</div>
					<?php 
					$ph_count++;
					} ?>
						<div class="modal-body clearfix">
							<!------------------------CAPTION START--------------------------------------------->
							<?php 
								foreach($privateGalleryArray2 as $gallery2){
									$ph_id=$gallery2['photo_id'];
									if($privatePhotoId2 == $ph_id) $cap_class='block'; else $cap_class='none';
							?>
								<div class="margin-t-15 pull-left Full-width photo_caption photo_caption-<?php echo $privatePhotoId2.'-'.$iCount;?>" id="caption-<?php echo $ph_id.'-'.$iCount;?>" style="display:<?php echo $cap_class;?>">
							...
							<!--------------PHOTO SLIDER START-------------------------->
								...
								<ul class="myphotosfix" id="bxslider-<?php echo $privatePhotoId2.'-'.$iCount;?>" >
									<?php foreach($privateGalleryArray2 as $gallery3){?>
										<li>
											<img src="<?php echo $userProfileObj->getPhotoPath($gallery3['photo']);?>" id="sliderImage-<?php echo $gallery3['photo_id'].'-'.$iCount;?>"/>
										</li>
									<?php } ?>
								</ul>
								<ul class="myphotosthum clearfix" id="bxslider-pager-<?php echo $privatePhotoId2.'-'.$iCount;?>">
									<?php 
									$iC=0;
									foreach($privateGalleryArray2 as $gallery4){?>
							...
							<?php 
							$com_class='';
							foreach($privateGalleryArray2 as $gallery5){
								$p_id=$gallery5['photo_id'];
								
								if($privatePhotoId2 == $p_id) $com_class='block'; else $com_class='none';
							?>
								<div class="popup-right-box popup-right-box-<?php echo $privatePhotoId2.'-'.$iCount;?>" id="popup-right-box-<?php echo $p_id.'-'.$iCount;?>" style="display:<?php echo $com_class;?>">
									<!--<div>
							...
							<!-------------------------COMMENT START------------------------------------>
							<div class="photos_comments photos_comments-<?php echo $privatePhotoId2.'-'.$iCount;?>" id="comment-<?php echo $p_id.'-'.$iCount;?>" style="display:<?php echo $com_class;?>">
								<form>
		...
		<div id="deletePhoto-<?php echo $privatePhotoId2; ?>" class="modal fade login_pop" role="dialog" >
						...
						<div class="modal-body clearfix">
							<span class="login-form">
								...
								<form action="" method="post">
									...
									<a href="photos.php?action=deletePhoto&photo_id=<?php echo $privatePhotoId2; ?>" class="green">Delete</a>
									</div>
								</form>
							</span>
						</div>
